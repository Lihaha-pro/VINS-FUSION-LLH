...
map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> FeatureTracker::trackImage(double _cur_time, const cv::Mat &_img, const cv::Mat &_img1)
{
    ...
    */
    ...
    if (prev_pts.size() > 0) 
    {
        ...
        // reverse check 反向检查
        // Step 反向检查 所谓反向检查就是用前后两帧调换位置进行光流追踪，判断追踪到的点是否是同一个
        if(FLOW_BACK)//
        {
            ...
            for(size_t i = 0; i < status.size(); i++)
            {
                ...
                if(status[i] && reverse_status[i] && distance(prev_pts[i], reverse_pts[i]) <= 0.5)//?最后的0.5判断条件有意义吗，这个函数会改变reverse_pts吗？
                {
                    status[i] = 1;
                }
                else
    ...
    if (1)
    {
        ...
        TicToc t_m;
        // Step 根据已有的特征，以及特征点最小间距，在图片上设置mask，实现特征提取均匀性
        setMask();//根据特征点最小间距设置mask，更新当前帧的特征点
        ...
        for (auto &p : n_pts)
        {
            cur_pts.push_back(p);
            ids.push_back(n_id++);
            track_cnt.push_back(1);//这些特征是新提取出来的，所以观察次数赋值为1
        }
        //printf("feature cnt after add %d\n", (int)ids.size());
    }
    // Step 对所有点进行去畸变，得到去畸变后的归一化平面相机坐标系坐标
    cur_un_pts = undistortedPts(cur_pts, m_camera[0]); //当前帧去畸变后的点
    // Step 计算特征点移动速度（是对去畸变后的坐标计算的）
