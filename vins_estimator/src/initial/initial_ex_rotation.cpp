...
bool InitialEXRotation::CalibrationExRotation(vector<pair<Vector3d, Vector3d>> corres, Quaterniond delta_q_imu, Matrix3d &calib_ric_result)
{
    ...
    Rc.push_back(solveRelativeR(corres));//相邻两帧图片的旋转
    Rimu.push_back(delta_q_imu.toRotationMatrix());//imu得到的旋转FIXME:这里和第七讲的L和R写反了
    Rc_g.push_back(ric.inverse() * delta_q_imu * ric);//将IMU得到的旋转转换到cam系下，用来计算核函数
    ...
    for (int i = 1; i <= frame_count; i++)
    {
        ...
        double w = Quaterniond(Rc[i]).w();//四元数实部，实部是最后一维
    ...
    Quaterniond estimated_R(x);///这里得到的是q_ci，下边转换为矩阵时求了逆
...
Matrix3d InitialEXRotation::solveRelativeR(const vector<pair<Vector3d, Vector3d>> &corres)
{
    if (corres.size() >= 9)//大于9个点时
    {
        ...
        */   
        ...
        if (determinant(R1) + 1.0 < 1e-09)//计算行列式
        {
            E = -E;
            decomposeE(E, R1, R2, t1, t2);
        } 
...
{
    cv::Mat pointcloud;//三角化后4×N的矩阵，每一列保存
...
//对应十四讲第一版的147页,对本质矩阵E进行SVD分解,求解出两个旋转和两个位移
