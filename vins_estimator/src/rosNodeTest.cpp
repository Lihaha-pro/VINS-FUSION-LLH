...
queue<sensor_msgs::ImuConstPtr> imu_buf;///根本没有用到哎，下一个变量也是
...
void sync_process()
{
    while(1)//始终循环，没有跳出
    {
        ...
        else
        {
            ...
            double time = 0;
            m_buf.lock();
            ///看来VINS-Fusion不再单独让特征提取节点订阅原始图片，而是直接查看img_buf里面有没有图片，有图片就进行处理
            if(!img0_buf.empty())
            {
                time = img0_buf.front()->header.stamp.toSec();
                header = img0_buf.front()->header;
                image = getImageFromMsg(img0_buf.front());//获取图片，保存为Mat类型
                img0_buf.pop();
            }
            m_buf.unlock();
            if(!image.empty())
                estimator.inputImage(time, image);///处理图片的入口
        }
...
void feature_callback(const sensor_msgs::PointCloudConstPtr &feature_msg)
{
    ...
    estimator.inputFeature(t, featureFrame);//将读入的feature和时间戳送入estimator
    return;
}
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    ros::Subscriber sub_feature = n.subscribe("/feature_tracker/feature", 2000, feature_callback);//订阅特征话题，解析特征数据后送入estimator
    ros::Subscriber sub_img0 = n.subscribe(IMAGE0_TOPIC, 100, img0_callback);///图片回调函数，用来读取图片数据存入img0_buf
