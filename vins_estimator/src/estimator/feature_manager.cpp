...
/**
 * @brief 更新特征点管理器，根据视差判断边缘化倒数第二帧还是最老帧
 * 
 * @param frame_count 
 * @param image 
 * @param td 
 * @return true 当前帧看到特征点视差与前两帧较大，返回true，边缘化最老的帧
 * @return false 边缘化倒数第二帧
 */
    ...
    // image 特征点id 图像id(0或1) xyz_uv_vel
{
    ...
    {
        FeaturePerFrame f_per_fra(id_pts.second[0].second, td);//每个特征点创建一个该对象，传入该特征信息以及时间戳

        assert(id_pts.second[0].first == 0);//判断单目，如果它的条件返回错误，则终止程序执行
        // 如果双目
        ...
        if (it == feature.end())
        {
            ...
            feature.back().feature_per_frame.push_back(f_per_fra);//添加该特征点的帧观测
            new_feature_num++;
        }

        //如果找到了相同ID特征点，就在其FeaturePerFrame内增加此特征点在此帧的位置以及其他信息，
        // 然后增加last_track_num，说明此帧有多少个相同特征点被跟踪到
        else if (it->feature_id == feature_id)//?这个判断不是多余吗，上边寻找it的时候就用的这个条件判断的呀
        {
            it->feature_per_frame.push_back(f_per_fra);//给该特征点添加该帧的观测
            last_track_num++;
    ...
    // Step 计算倒数二三帧共同看到的特征点视差，进而判断倒数第二帧是否作为关键帧
    ...
    else
    {
        ...
        last_average_parallax = parallax_sum / parallax_num * FOCAL_LENGTH;///好像没有用到这个变量
...
void FeatureManager::triangulate(int frameCnt, Vector3d Ps[], Matrix3d Rs[], Vector3d tic[], Matrix3d ric[])
{
    for (auto &it_per_id : feature)//对于所有的特征点
    {
        ...
        else if(it_per_id.feature_per_frame.size() > 1)
        {
            int imu_i = it_per_id.start_frame;
            Eigen::Matrix<double, 3, 4> leftPose;
            Eigen::Vector3d t0 = Ps[imu_i] + Rs[imu_i] * tic[0];//?这俩坐标是什么东西
            Eigen::Matrix3d R0 = Rs[imu_i] * ric[0];
            leftPose.leftCols<3>() = R0.transpose();
            leftPose.rightCols<1>() = -R0.transpose() * t0;
