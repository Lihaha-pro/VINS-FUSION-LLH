...
void Estimator::setParameter()
{
    ...
    // 将相机到imu外参传入（如何没有先验，这里就是单位阵和0平移）
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        tic[i] = TIC[i];
        ric[i] = RIC[i];
...
{        
    ...
    // 特征追踪所用的时间 这个很好用啊！！
    // Step 进行光流跟踪得到特征信息featureFrame
    if(_img1.empty())
        featureFrame = featureTracker.trackImage(t, _img);//llh 追踪单目 //?这里的featureFrame和主函数中什么区别
    else
        featureFrame = featureTracker.trackImage(t, _img, _img1);// 追踪双目
    printf("featureTracker time: %f\n", featureTrackerTime.toc());

    if (SHOW_TRACK)//这个是展示轨迹，获得标记了特征的图片（专门用于可视化），然后发布出去，话题名为track_image
    {
...
/**
 * @brief 对imu的时间进行判断，将队列里的imu数据放入到accVector和gyrVector中，完成之后返回true
 * 
 * @param t0 上一帧图片时间
 * @param t1 当前帧图片时间
 * @param accVector 
                                ...
                                vector<pair<double, Eigen::Vector3d>> &gyrVector)
{
    ...
    if(t1 <= accBuf.back().first)
    {
        ...
        //这里得到的是从上一帧图片时间之后到当前帧图片时间之后一个的IMU数据
...
void Estimator::processMeasurements()
{
    while (1)
    {
        ...
        //一帧图片的全部特征信息，包括：时间戳 特征点ID 图像id xyz_uv_vel 处理特征点的buf
        pair<double, map<int, vector<pair<int, Eigen::Matrix<double, 7, 1> > > > > feature;
        //
        vector<pair<double, Eigen::Vector3d>> accVector, gyrVector;//保存用于和当前帧耦合的IMU数据
        
        if(!featureBuf.empty())
        {            
            feature = featureBuf.front();//.front()返回当前vector容器中起始元素的引用。
            curTime = feature.first + td;//当前帧对应到imu的时间，td的使用是在图像的时间上加上这个值
            //等待IMU数据到来
            while(1)
            {
...
void Estimator::initFirstIMUPose(vector<pair<double, Eigen::Vector3d>> &accVector)
{
    ...
    R0 = Utility::ypr2R(Eigen::Vector3d{-yaw, 0, 0}) * R0;//令初始的航向为0 //?怎么又执行了一遍
    Rs[0] = R0;
...
void Estimator::processIMU(double t, double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity)
{
    ...
    if (!first_imu)
    {
        ...
        gyr_0 = angular_velocity;//gyr表示gyroscope，陀螺仪的意思
    }
    ...
    if (frame_count != 0)
    {
        ...
        int j = frame_count;         
        Vector3d un_acc_0 = Rs[j] * (acc_0 - Bas[j]) - g;//移除了偏置的加速度
        Vector3d un_gyr = 0.5 * (gyr_0 + angular_velocity) - Bgs[j];//移除了偏置的gyro
...
/**
 * @brief 处理图片：其中包含了检测关键帧,估计外部参数,初始化,状态估计,划窗等等
 * 
 * @param image 图片特征信息
 * @param header 该帧图片时间戳
 */
...
// image 里边放的就是该图像的特征点 header 时间
{
    ...
    ImageFrame imageframe(image, header);//构造一个图像对象，包含特征、位姿、预积分量、是否是关键帧的标志
    ...
    if(ESTIMATE_EXTRINSIC == 2)
    {
        ...
        if (frame_count != 0)
        {
            ///获得相邻两帧的特征匹配（归一化相机坐标），准备后边进行旋转外参初始化
...
void Estimator::fastPredictIMU(double t, Eigen::Vector3d linear_acceleration, Eigen::Vector3d angular_velocity)
{
    ...
    latest_time = t;
    Eigen::Vector3d un_acc_0 = latest_Q * (latest_acc_0 - latest_Ba) - g;//上一时刻在世界系下的加速度值
    Eigen::Vector3d un_gyr = 0.5 * (latest_gyr_0 + angular_velocity) - latest_Bg; //角速度均值
    latest_Q = latest_Q * Utility::deltaQ(un_gyr * dt);//更新当前的角度
    Eigen::Vector3d un_acc_1 = latest_Q * (linear_acceleration - latest_Ba) - g;//当前时刻世界系下加速度
    Eigen::Vector3d un_acc = 0.5 * (un_acc_0 + un_acc_1);//加速度均值
    latest_P = latest_P + dt * latest_V + 0.5 * dt * dt * un_acc;//更新当前的位置
    latest_V = latest_V + dt * un_acc;//更新当前的速度
