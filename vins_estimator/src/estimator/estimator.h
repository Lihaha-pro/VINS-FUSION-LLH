...
class Estimator
{
  public:
    ...
    //队列每个元素为某帧时间戳（double），以及特征信息
    queue<pair<double, map<int, vector<pair<int, Eigen::Matrix<double, 7, 1> > > > > > featureBuf;
    double prevTime, curTime;//帧对应的IMU时间
    ...
    map<double, ImageFrame> all_image_frame; //保存完整图片信息的数据结构
    IntegrationBase *tmp_pre_integration;//这个是输入到图像中的预积分值（用来维护图片管理器）
