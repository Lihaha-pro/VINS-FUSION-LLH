...
void readParameters(std::string config_file)
{
    ...
    // 设置相机参数
    std::string cam0Calib;//得到相机配置文件的名字
    fsSettings["cam0_calib"] >> cam0Calib;
    std::string cam0Path = configPath + "/" + cam0Calib;//整合得到配置文件的完整路径
    CAM_NAMES.push_back(cam0Path);//放的是含有相机参数的文件，因为如果两个相机，所以用了vector
    ...
    //是否在线估计时间
    TD = fsSettings["td"];
    ESTIMATE_TD = fsSettings["estimate_td"];//是否进行延时估计的标志位
    if (ESTIMATE_TD)
